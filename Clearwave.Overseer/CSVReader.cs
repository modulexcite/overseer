...
namespace Clearwave.Overseer
{
    public class CSVReader : IDisposable, IDelimitedReader, IEnumerable<IDelimitedRow>
    {
        ...
        public CSVReader(TextReader reader, bool reuseRow, char delimiter = ',')
        {
            this.reader = reader;
        ...
        public CSVReader(Stream stream, bool reuseRow, char delimiter = ',')
            : this(new StreamReader(stream), reuseRow, delimiter) { }
        ...
        public IDelimitedRow NextRow()
        {
            ...
            for (int i = 0; i < line.Length; i++)
            {
                ...
                if (line[i] == delimiter && !inDoubledQuotedField)
                {
